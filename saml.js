...
let ASSERTION_NS = 'urn:oasis:names:tc:SAML:2.0:assertion'
...
exports.parseRequest = function (options, request, callback) {
  ...
  zlib.inflateRaw(buffer, function (err, result) {
    ...
    try {
      ...
      let doc = new Parser().parseFromString(buffer.toString())
      ...
      if (rootElement.localName == 'AuthnRequest') {
        ...
        let issuer = rootElement.getElementsByTagNameNS(ASSERTION_NS, 'Issuer')[0]
        if (issuer) {
          info.login.issuer = issuer.textContent
        }
        let nameIDPolicy = rootElement.getElementsByTagNameNS(SAMLP_NS, 'NameIDPolicy')[0]
        if (nameIDPolicy) {
          info.login.nameIdentifierFormat = nameIDPolicy.getAttribute('Format')
        }
        let requestedAuthnContext = rootElement.getElementsByTagNameNS(SAMLP_NS, 'RequestedAuthnContext')[0]
        if (requestedAuthnContext) {
          let authnContextClassRef = requestedAuthnContext.getElementsByTagNameNS(ASSERTION_NS, 'AuthnContextClassRef')[0]
...
exports.createAssertion = function (options) {
  ...
  if (options.issuer) {
    let issuer = doc.documentElement.getElementsByTagName('saml:Issuer')
  ...
  if (options.lifetimeInSeconds) {
    let expires = new Date(Date.now() + options.lifetimeInSeconds * 1000).toISOString()
  ...
  if (options.audiences) {
    let audienceRestrictionsElement = doc.createElementNS(ASSERTION_NS, 'saml:AudienceRestriction')
    var audiences = options.audiences instanceof Array ? options.audiences : [options.audiences]
    audiences.forEach(function (audience) {
      let element = doc.createElementNS(ASSERTION_NS, 'saml:Audience')
  ...
  if (options.attributes) {
    let statement = doc.createElementNS(ASSERTION_NS, 'saml:AttributeStatement')
    ...
    Object.keys(options.attributes).forEach(function (prop) {
      ...
      values.forEach(function (value) {
        let valueElement = doc.createElementNS(ASSERTION_NS, 'saml:AttributeValue')
  ...
  let nameID = doc.documentElement.getElementsByTagNameNS(ASSERTION_NS, 'NameID')[0]
  ...
  if (options.authnContextClassRef) {
    let authnCtxClassRef = doc.getElementsByTagName('saml:AuthnContextClassRef')[0]
    authnCtxClassRef.textContent = options.authnContextClassRef
  }

  let sig = exports.signDocument(doc.toString(), "//*[local-name(.)='Assertion']", options)
